// include guard untuk menghindari error redefinition
...
// struktur simpul senarai
struct Node {
    char data;          // field data berisi informasi dari simpul
    Node *next = NULL;  // field penyambung simpul
};

// struktur senarai
struct List {
    Node *head = NULL;  // simpul head yang selalu menunjuk ke simpul pertama
    Node *tail = NULL;  // simpul tail yang selalu menunjuk ke simpul terakhir
};

/**
 *  @brief prosedur untuk menambahkan simpul di awal senarai
 *  @param senarai List*
 *  @param data char
 */
void InsertFirst(List *senarai, char data) {
    ...
    // senarai tidak mempunyai simpul atau kosong
